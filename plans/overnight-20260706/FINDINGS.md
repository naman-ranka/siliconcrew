...
| X2M-2 (all 5 PD-summary tools -32602 on a completed run, incl. no-arg get_cts_summary, while status/search/report worked same-minute) | HIGH | SCHEMA HYPOTHESIS DISPROVEN (29d1535): the failing tools' schemas are byte-identical (mod title) to generate_report_tool's, which worked same-minute — so not schema shape; our call_tool never emits -32602. Real cause = the F9c pre-init mis-map firing under the X2M-1 degradation (temporal, not signature, family). Both fixes (stateless cad3242, F2 gating f095fcb) ship tonight. Durable guard kept: every MCP tool's schema validated well-formed + canonical-payload in tests/test_mcp_tool_schemas.py. UNVERIFIED owner-flagged hypothesis: PD tools read ORFS report FILES → may force full GCS workspace materialization on hosted (get_synthesis_status reconciles from the run store instead); if post-deploy PD calls are still slow/failing on big workspaces, the fix is a no-full-materialization read path (own small plan). POST-DEPLOY VERIFY: PD tools on x2_fir_mcp_20260707. |
...
| X2M-5 (schematic_tool leaks raw docker-socket error on hosted) | MED | FIXED (928d642): wrapper-entry hosted gate (mirrors run_python_analysis) returns an honest "not available on hosted yet — run self-host" message; no docker.sock string, no traceback; self-host unchanged. Hosted schematic SUPPORT remains an owner feature decision. |
