...
3. Architecture variants
   Before choosing a variant, read the critical path of the tightest passing run
   (search_logs_tool "data arrival time", "startpoint", "endpoint" in 6_finish.rpt or
   2_floorplan_final.rpt). Let that tell you where the bottleneck actually is —
   the right variant breaks the bottleneck, not just changes the style.

   For example: if the critical path lives in the accumulation tree, adding a pipeline
   register mid-tree is more interesting than removing pipeline registers elsewhere.
   If the bottleneck is a multiplier, pre-registering inputs or retiming may help.
   Use your judgment — the goal is a variant that has a realistic chance of a distinct
   Pareto point, not coverage for its own sake.

   Only pursue a variant further if it shows a distinct Pareto point — better area OR
   better Fmax at same or lower power.
...
- Architecture: variant is strictly dominated (worse area AND worse power AND worse Fmax
  than the baseline) — document why and exclude from frontier.
