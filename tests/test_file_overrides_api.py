"""W3 (L1): optional file overrides on the core lint/simulate/synthesize
commands — REST twins AND the sim wrapper — resolved through the ONE shared
resolver (amendment A13), with honest notes when an override drops a
manifest-supplied file (A27) and the synthesize .v/.sv filter kept (A14).
...
def test_run_isolated_simulation_wrapper_override(tmp_path, monkeypatch):
    ...
    out = json.loads(wrappers.run_isolated_simulation.func(verilog_files=["counter_tb.v"]))
    assert seen["verilog_files"] == ["tb/counter_tb.v"]
...
def test_run_isolated_simulation_wrapper_without_override_unchanged(tmp_path, monkeypatch):
    wrappers = pytest.importorskip("src.tools.wrappers")
    ws = str(tmp_path)
    _seed_nested(ws)
    monkeypatch.setattr(wrappers, "get_workspace_path", lambda: ws)
    ...
    monkeypatch.setattr(wrappers, "run_sim_isolated", fake_sim)
    out = json.loads(wrappers.run_isolated_simulation.func())
    assert sorted(seen["verilog_files"]) == ["rtl/counter.v", "tb/counter_tb.v"]
    assert "manifestWarnings" not in out


def test_run_isolated_simulation_wrapper_override_bad_file(tmp_path, monkeypatch):
    wrappers = pytest.importorskip("src.tools.wrappers")
    ws = str(tmp_path)
    _seed_nested(ws)
    monkeypatch.setattr(wrappers, "get_workspace_path", lambda: ws)
    out = wrappers.run_isolated_simulation.func(verilog_files=["ghost.v"])
    assert out.startswith("Error:") and "does not exist" in out


def test_twins_and_wrappers_share_one_resolver():
    """A13: parity is the SAME helper object, not two lookalikes."""
    from src.tools import file_resolver
    ...
    assert actions_mod.override_drop_notes is file_resolver.override_drop_notes
    assert wrappers.override_drop_notes is file_resolver.override_drop_notes
