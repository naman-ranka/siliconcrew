...
| `WORKOS_CLIENT_ID` (+ `WORKOS_REDIRECT_URI`; MCP service also `WORKOS_AUDIENCE` + `MCP_RESOURCE_URL`) | remote-MCP auth + web/MCP unification (hosted only; leave empty to keep Google sign-in). Issuer/JWKS are derived from the client id. See `deploy/MCP_REMOTE_AUTH.md` |
