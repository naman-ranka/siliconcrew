...
import { extractErrorCode, extractErrorDetails, extractErrorMessage, isSignInRequired } from "@/lib/api";
...
// FA8: the `_err` envelope's structured details reach thrown errors, so a
// 400 invalid_arguments lights up field-level messages in the Surface.
describe("extractErrorDetails", () => {
  it("reads details from the envelope-in-HTTPException and top-level shapes", () => {
    const fields = [{ field: "run_id", message: "unknown run" }];
    expect(
      extractErrorDetails({ detail: { ok: false, error: { code: "invalid_arguments", details: { fields } } } })
    ).toEqual({ fields });
    expect(extractErrorDetails({ ok: false, error: { code: "x", details: { fields } } })).toEqual({ fields });
  });
  it("is undefined for detail-less shapes", () => {
    expect(extractErrorDetails({ detail: { code: "signin_required", message: "m" } })).toBeUndefined();
    expect(extractErrorDetails({ detail: "plain" })).toBeUndefined();
