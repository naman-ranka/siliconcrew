...
describe("dashboard artifact keys agree across every entry point", () => {
  ...
  it("write_file, both edit_file forms and the file tree all yield ONE key", () => {
    const expected = `interactive:${FILE}`;
    expect(artifactKeyForToolCall("write_file", { filename: FILE })).toBe(expected);
    expect(artifactKeyForToolCall("edit_file", { filename: FILE })).toBe(expected);
    // the diff form must not reintroduce the dual-tab-key bug (code: vs interactive:)
    expect(
  ...
  it("non-dashboard writes still open as code", () => {
    ...
    expect(artifactKeyForToolCall("edit_file", { filename: "alu.v" })).toBe("code:alu.v");
  });
