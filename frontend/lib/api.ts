...
/**
 * The machine-readable error CODE across every backend error shape (W4/A17):
...
/** Errors thrown by apiFetch/actionFetch carry the HTTP status and the
 *  backend's error code when the body supplied one. */
...
async function apiFetch<T>(
  ...
  if (!response.ok) {
    ...
    const err = new Error(extractErrorMessage(error, "API request failed")) as ApiError;
...
async function actionFetch<T>(endpoint: string, options?: RequestInit): Promise<T> {
  ...
  if (!response.ok || (body && body.ok === false)) {
    // Same status+code attachment as apiFetch (W4/A17): the Command Surface
    // detects signin_required by CODE across the 403-detail (core twins) and
    // 401-envelope (/invoke) shapes.
    const err = new Error(
      extractErrorMessage(body, response.statusText || "Action failed")
    ) as ApiError;
    err.status = response.status;
    err.code = extractErrorCode(body);
    throw err;
  }
  return body as T;
}
...
export const workbenchApi = {
  ...
  // `files` (W3/L1): optional override of the manifest-resolved compile set —
  ...
  // never alter the run result). `files` (W3/L1): optional compile-set
  // override; absent = files_for_stage(manifest) as today.
  simulate: (
    sessionId: string,
