// AES inverse S-box (byte substitution for AES decryption).
//
// Adapted from the Tiny Tapeout project "AES inverse S-box"
...
//
// Adaptation: the upstream Tiny Tapeout wrapper (tt_um_daosvik_aesinvsbox) maps
// this core onto the fixed ui_in/uo_out/uio pins. This bundle drops that wrapper
// and uses the core module `sbox_aesinv` directly as the top, verbatim. Its
// standard-cell primitives are supplied by the portable behavioral library in
// `sky130.v` (kept as-is except that the upstream `keep_hierarchy` attribute is
// removed so the standard flow flattens and reports true PPA — see sky130.v).
// The design lints, simulates, and synthesizes without a PDK. Ports: clk,
// x[7:0] in -> y[7:0] combinational, cy[7:0] registered. The self-checking
// testbench is original to this bundle.
//
/*
