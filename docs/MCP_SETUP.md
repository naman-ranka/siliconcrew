...
```json
{
  "mcpServers": {
    "rtl-design-agent": {
      ...
      "args": [
        "C:\\<path-to-RTL_AGENT>\\mcp_server.py"
      ],
      "env": {
...
- `cocotb_tool` - Run a cocotb (Python) testbench in the pinned simulator container
- `sby_tool` - Formal verification (SymbiYosys: bounded model check or proof)
...
Nothing is deleted — they are always available in the web Command Surface, and
a server started with `--codex-tools` advertises them too.
